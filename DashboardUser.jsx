...
export const DashboardUserCard = ({ data, index }) => {
  ...
  return (
    <motion.div
      key={index}
      className='relative w-full rounded-md flex items-center justify-between py-4 bg-lightOverlay cursor-pointer hover:bg-card 
      hover:shadow-md'
    >
      ...
      <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative'>
...

        {
          data._id !== user?.user._id && (
            <motion.p whileTap={{ scale: 0.75 }}
              className='text-[10px] font-semibold text-textColor2 px-1 bg-purple-200 rounded-sm hover:shadow-md'
        ...
        {isUserRoleUpdated && (
          <motion.div
            ...
            className="absolute z-10 top-6 right-4 p-4 flex items-center flex-col gap-4 bg-black shadow-xl rounded-md">
            <p className='text-textColor text-[12px] font-semibold'>
              Bạn có chắc đánh dấu người dùng này là <span>{data.role == "admin" ? "Member" : "Admin"}</span> ?
            </p>
